...
const INITIAL_FIXED_STEP_DELAY_MS = 250; // Was 201
const DELAY_BEFORE_ACCELERATION_START_MS = 500; // Was 200
const ACCELERATION_PHASE_INITIAL_DELAY_MS = 150; // Was 70
const MIN_STEP_DELAY_MS = 25; // Was 15
const ACCELERATION_FACTOR = 0.92; // Was 0.90

const MIN_INPUT_WIDTH_REM = 7; 
const MIN_SLIDER_WIDTH_REM = 3; 

const MIN_RESERVED_WIDTH_FOR_INPUT_TEXT_PX = 75;
const UNIT_VISIBILITY_PADDING_PX = 16; // Extra padding for unit visibility check
...
export const Input: React.FC<InputProps> = ({
  ...
  const overallContainerRef = useRef<HTMLDivElement>(null); // Ref for the main container
  const internalUnitRef = useRef<HTMLSpanElement>(null); // Ref for the internal unit span
  ...
  useLayoutEffect(() => {
    if (!numeric && unit && overallContainerRef.current) {
      ...
      setShowInternalUnit(true); // Ensure unit is shown if conditions for observer don't apply
    }
  }, [numeric, unit]); // Re-run if numeric or unit changes, to re-attach/detach observer or reset visibility
  ...
  }, [currentSliderMin, currentSliderMax, showSlider, onChange, formatValueToStepPrecision, clampValue, props.disabled]);
